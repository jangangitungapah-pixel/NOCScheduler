  ...
  return (
    ...
    >
      ...
      <div className="product-agenda-row__main">
        ...
        <p>
          {item.note ??
            (item.state === "PLANNED" ? "Published planned assignment" : "Effective assignment")}
        </p>
      </div>
    ...
    >
      ...
      <section className="product-dashboard-lower">
        ...
        <div>
          <SectionHeading
            action={
              <Link className="product-text-link" href="/activity">
                View history
              </Link>
            }
...
function ScheduleHubSurface({ meta }: { meta: RouteMeta }) {
  const cards = [
    [
      "My Schedule",
      "Personal agenda, week, month, and change context.",
      "/schedule/me",
      "calendar",
    ],
    ...
    [
      "Manage Schedule",
      "Draft, validate, bulk-edit, and publish.",
      "/schedule/manage",
      "panel-left",
    ],
    [
      "Requests",
      "Leave, swap, replacement, overtime, and permission.",
      "/schedule/requests",
      "file-text",
    ],
  ] as const;
      ...
      <section className="product-two-column">
        <div>
          ...
          <Surface className="product-agenda" padding="none">
            {mySchedule.map((item) => (
              <ShiftLine item={item} key={item.workDate} />
            ))}
          </Surface>
        </div>
        <aside className="product-side-stack">
          ...
          <Surface padding="md">
            ...
            <p className="product-muted">
              Latest published correction changed 17 Aug from S2 to S3.
            </p>
            <Link className="product-text-link" href="/activity">
              Open change history →
            </Link>
    ...
    <ProductPage
      actions={
        <>
          ...
          <HeaderAction href="#publish-review" primary>
            Review & publish
          </HeaderAction>
        </>
      }
      ...
      <section className="product-manage-layout">
        ...
        <aside className="product-validation-panel">
          <Surface padding="md">
            ...
            <div className="product-side-focus">
              <ShiftBadge expanded shift="S1" />
              <strong>Apply Shift 1</strong>
          ...
          <Surface padding="md">
            ...
            <div className="product-finding">
              ...
              <div>
                ...
                <p>All required S1 slots remain covered.</p>
              </div>
            </div>
          </Surface>
          <Surface id="publish-review" padding="md">
            ...
            <ul className="product-compact-list">
              <li>3 assignments changed</li>
              <li>1 employee notified</li>
              <li>Payroll source will become dirty</li>
            </ul>
            <button
              className="product-action product-action--primary product-action--full"
              type="button"
            >
              Publish revision 04
            </button>
...
function RequestsSurface({ meta, pathname }: { meta: RouteMeta; pathname: string }) {
  ...
  return (
    <ProductPage
      actions={
        <HeaderAction href="/schedule/requests?create=1" primary>
          New request
        </HeaderAction>
      }
      meta={meta}
    >
      <section className="product-list-detail">
        <div>
          ...
          <Surface className="product-request-list" padding="none">
            {requests.map((request) => (
              <Link
                className={`product-request-row${request.id === selected.id ? " is-selected" : ""}`}
        ...
        <aside
          className={
            detailMode
              ? "product-detail-panel product-detail-panel--active"
        ...
        >
          <Surface padding="lg">
            <div className="product-detail-head">
              <div>
                <span className="product-eyebrow">{selected.id}</span>
                <h2>{requestTypeLabel(selected.type)} request</h2>
              </div>
            ...
            <dl className="product-definition-grid">
              ...
              <div>
                <dt>Source state</dt>
                <dd>Published schedule</dd>
              </div>
              <div>
                <dt>Payroll impact</dt>
                <dd>Explicit on approval</dd>
              </div>
            </dl>
            ...
            <div className="product-approval-steps">
              <span className="is-done">Submitted</span>
              <span className={selected.status !== "PENDING" ? "is-done" : "is-current"}>
                Supervisor decision
              </span>
...
function EmployeeListSurface({ meta }: { meta: RouteMeta }) {
  return (
    <ProductPage
      actions={<HeaderAction href="/settings/access">Manage access</HeaderAction>}
      meta={meta}
    >
      <div className="product-toolbar">
        <label className="product-search">
          <Icon name="search" size={16} />
          <input aria-label="Search employees" placeholder="Search employee" />
        </label>
        <Badge tone="neutral">{employees.length} active people</Badge>
      </div>
      <Surface className="product-directory" padding="none">
        {employees.map((employee) => (
          <Link
            className="product-directory-row"
          ...
          >
            ...
            <div className="product-directory-row__meta">
              <span>Aug shifts 20</span>
              <span>S2 6 · S3 5</span>
            </div>
            <Icon name="chevron-right" size={18} />
          </Link>
...
function EmployeeDetailSurface({ meta, pathname }: { meta: RouteMeta; pathname: string }) {
  ...
  const activeTab = pathname.endsWith("/schedule")
    ? "schedule"
    : pathname.endsWith("/payroll")
      ? "payroll"
  ...
  return (
    <ProductPage meta={meta}>
      <Surface className="product-profile-hero" padding="lg">
        <Avatar initials={employee.initials} name={employee.name} />
        <div>
          <span className="product-eyebrow">Employee profile</span>
          <h2>{employee.name}</h2>
          <p>{employee.role} · Internal NOC</p>
        </div>
      ...
      <nav className="product-tabs" aria-label="Employee detail tabs">
        ...
        <Link
          ...
          href={`/employees/${employee.id}/schedule`}
        >
          Schedule
        </Link>
        <Link
      ...
      {activeTab === "overview" ? (
        <div className="product-summary-grid">
          <Surface padding="md">
            <SectionHeading eyebrow="August" title="Operational summary" />
            <div className="product-summary-strip product-summary-strip--embedded">
              <Stat label="Assignments" value="20" />
              <Stat label="S2" value="6" />
              <Stat label="S3" value="5" />
            </div>
          </Surface>
          <Surface padding="md">
            <SectionHeading eyebrow="Next" title="Upcoming shift" />
            <div className="product-side-focus">
              <ShiftBadge expanded shift="S2" />
              <strong>15:00–23:00</strong>
              <span>14 Aug · Main NOC Floor</span>
      ...
      {activeTab === "payroll" ? (
        <PayrollBreakdown
          employee={payrollRows.find((row) => row.employeeId === employee.id) ?? payrollRows[0]!}
        />
      ) : null}
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.employeeId}>
              <td>
                <Link href={`/payroll/2026-08/${row.employeeId}`}>{row.employeeName}</Link>
              </td>
              <td>{money.format(row.baseSalary)}</td>
              <td>
                {row.s2Count} · {money.format(row.s2Amount)}
              </td>
              <td>
                {row.s3Count} · {money.format(row.s3Amount)}
              </td>
              <td>{money.format(row.adjustments)}</td>
              <td>
                <strong>{money.format(row.takeHomePay)}</strong>
              </td>
              <td>
...
function PayrollBreakdown({ employee }: { employee: PayrollRowFixture }) {
  return (
    <section className="product-payroll-detail">
      <Surface className="product-payroll-total" elevation="raised" padding="lg">
        <span>Calculated take-home pay</span>
        <strong>{money.format(employee.takeHomePay)}</strong>
      ...
      <Surface padding="none">
        ...
        <div className="product-breakdown-row">
          <span>Shift 2 incentive · {employee.s2Count} shifts</span>
          <strong>{money.format(employee.s2Amount)}</strong>
        </div>
        <div className="product-breakdown-row">
          <span>Shift 3 incentive · {employee.s3Count} shifts</span>
          <strong>{money.format(employee.s3Amount)}</strong>
        </div>
        <div className="product-breakdown-row">
          <span>Manual adjustments</span>
          <strong>{money.format(employee.adjustments)}</strong>
        </div>
        <div className="product-breakdown-row product-breakdown-row--total">
          <span>Calculated THP</span>
          <strong>{money.format(employee.takeHomePay)}</strong>
        </div>
      </Surface>
      <Surface padding="md">
        <SectionHeading eyebrow="Source traceability" title="Why this number?" />
        <div className="product-source-line">
          <Icon name="calendar" size={16} />
          <div>
            <strong>Published schedule · August 2026</strong>
            <span>S2/S3 count uses work-date eligibility from revision 04.</span>
          </div>
        </div>
        <div className="product-source-line">
          <Icon name="history" size={16} />
          <div>
            <strong>Compensation version effective 1 Aug</strong>
            <span>Historical rates remain attached to this calculation.</span>
          </div>
        </div>
      </Surface>
    </section>
...
function PayrollSurface({ meta, pathname }: { meta: RouteMeta; pathname: string }) {
  ...
  if (segments.length >= 3) {
    ...
    return (
      <ProductPage
        actions={<HeaderAction href="/payroll/2026-08">Back to monthly payroll</HeaderAction>}
        meta={meta}
        workspace
      >
        <PayrollBreakdown employee={employee} />
      </ProductPage>
    );
  }
  ...
  return (
    <ProductPage
      actions={<HeaderAction href="/reports/payroll">Open payroll report</HeaderAction>}
      meta={meta}
      workspace
    >
      <div className="product-payroll-period">
        <div>
          <span className="product-eyebrow">August 2026</span>
          <h2>{isMonthly ? "Monthly payroll review" : "Current payroll period"}</h2>
        </div>
        <div className="product-inline-meta">
          <Badge showDot tone="warning">
            DIRTY
          </Badge>
          <span>Source changed 16:55</span>
        </div>
      </div>
      <div className="product-summary-strip product-summary-strip--cards">
        <Stat
          label="Total calculated THP"
          value={compactMoney.format(payrollTotal())}
          detail="5 employee fixture rows"
        />
        <Stat
          label="S2 incentive"
          value={money.format(payrollRows.reduce((sum, row) => sum + row.s2Amount, 0))}
          detail="31 eligible shifts"
        />
        <Stat
          label="S3 incentive"
          value={money.format(payrollRows.reduce((sum, row) => sum + row.s3Amount, 0))}
          detail="27 eligible shifts"
        />
        <Stat
          label="Adjustments"
          value={money.format(payrollRows.reduce((sum, row) => sum + row.adjustments, 0))}
          detail="manual, reason required"
        />
      </div>
...
function ReportBars() {
  ...
  return (
    <div className="product-chart" aria-label="Coverage chart fixture">
      {values.map((value, index) => (
        <div className="product-chart__column" key={index}>
          <div style={{ height: `${value}%` }} />
          <span>{11 + index} Aug</span>
        </div>
      ))}
    </div>
  );
...
function ReportsSurface({ meta, pathname }: { meta: RouteMeta; pathname: string }) {
  const reportName = pathname.endsWith("/payroll")
  ...
  return (
    <ProductPage
      actions={
        <>
          <HeaderAction href="#filters">Filters</HeaderAction>
          <HeaderAction href="#export" primary>
            Export
          </HeaderAction>
        </>
      }
      ...
      workspace
    >
      <div className="product-filter-bar" id="filters">
        <label>
          Period
        ...
        <label>
          View
          <select defaultValue="effective">
            <option value="effective">Effective state</option>
            <option value="planned">Planned state</option>
          </select>
        </label>
        <label>
          Employee
      ...
      <section className="product-report-layout">
        <Surface padding="lg">
          <SectionHeading eyebrow="August 11–17" title={reportName} />
          <ReportBars />
        </Surface>
        <Surface padding="md">
          <SectionHeading eyebrow="Interpretation" title="Decision support" />
          <p className="product-muted">
            Fairness is contextual guidance, not an automatic employee judgment. Exact values remain
            available beside every visualization.
          </p>
          <div className="product-finding product-finding--warning">
            <Icon name="warning" size={16} />
            <div>
              <strong>18 Aug · S3 gap</strong>
              <p>1 required slot remains uncovered.</p>
            </div>
          </div>
        </Surface>
      </section>
          ...
          <tbody>
            {[11, 12, 13, 14, 15].map((date, index) => (
              <tr key={date}>
                ...
                <td>{index === 4 ? 2 : 3}</td>
                <td>
                  <Badge tone={index === 4 ? "warning" : "success"}>
                    {index === 4 ? "66.7%" : "100%"}
                  </Badge>
                </td>
              </tr>
      ...
      <Surface className="product-export-panel" id="export" padding="md">
        <div>
          <strong>Export current report context</strong>
          <span>
            Includes generated time, Asia/Jakarta timezone, filters, report identity, and source
            revision metadata.
          </span>
        </div>
        <div className="product-button-row">
  ...
  return (
    <Surface className="product-activity-list" padding="none">
      {activityEvents.map((event) => (
        <div className="product-activity-row" key={event.id}>
          ...
          <div>
            ...
            {!compact ? <span>Reason: {event.reason}</span> : null}
          </div>
...
function ActivitySurface({ meta, pathname }: { meta: RouteMeta; pathname: string }) {
  ...
  return (
    <ProductPage meta={meta} workspace>
      <div className="product-toolbar">
        <label className="product-search">
          <Icon name="search" size={16} />
      ...
      <section className="product-list-detail">
        ...
        >
          <Surface padding="lg">
            <span className="product-eyebrow">AUD-8041</span>
            <h2>Published schedule correction</h2>
            <dl className="product-definition-grid">
              <div>
                <dt>Actor</dt>
                <dd>Yusuf Ramadhan</dd>
              </div>
              <div>
                <dt>Recorded at</dt>
                <dd>13 Aug · 17:22</dd>
            ...
            <div className="product-diff">
              <div>
                <span>Before</span>
                <strong>S2 · 15:00–23:00</strong>
              </div>
              <Icon name="chevron-right" size={20} />
              <div>
                <span>After</span>
                <strong>S3 · 23:00–07:00</strong>
              </div>
            </div>
            <div className="product-note">
              <strong>Reason</strong>
              <p>Holiday coverage adjustment.</p>
            </div>
          </Surface>
        </aside>
      </section>
...
function SettingsContent({ section }: { section: string }) {
  if (section === "shifts")
    return (
      <div className="product-setting-stack">
        {[
          ["Shift 1", "S1", "07:00", "15:00", "Rp0"],
          ["Shift 2", "S2", "15:00", "23:00", "Rp50.000"],
          ["Shift 3", "S3", "23:00", "07:00", "Rp75.000"],
        ].map(([name, code, start, end, incentive]) => (
          <Surface className="product-setting-row" key={code} padding="md">
            <ShiftBadge shift={code as ShiftKind} />
            <div>
              <strong>{name}</strong>
              <span>
                {start}–{end}
            ...
            <Badge tone="success">Effective 1 Aug</Badge>
          </Surface>
        ))}
      </div>
    );
  if (section === "compensation")
    return (
      <Surface padding="lg">
        <SectionHeading eyebrow="Effective-dated configuration" title="Compensation versions" />
        <div className="product-form-grid">
          <label>
            Employee
            <select defaultValue="emp-001">
              <option value="emp-001">Raka Pratama</option>
            </select>
          </label>
          <label>
            Base salary
          ...
          <label>
            Change reason
            <input defaultValue="Annual compensation review" />
          </label>
        </div>
        <div className="product-note">
          <strong>Historical impact</strong>
          <p>
            New effective versions change future calculations only. Existing historical references
            remain stable.
          </p>
        </div>
      </Surface>
      ...
      <Surface className="product-table-wrap" padding="none">
        <table className="product-table">
          ...
          <tbody>
            ...
            <tr>
              <td>Nadia Putri</td>
              <td>Scheduler/Supervisor</td>
              <td>Manage + Publish</td>
              <td>Read</td>
              <td>
    ...
    return (
      <Surface padding="none">
        <div className="product-setting-row">
          <div className="product-date-block">
            <strong>17</strong>
            <span>Aug</span>
          </div>
          <div>
            <strong>Hari Kemerdekaan Republik Indonesia</strong>
            <span>
              Holiday does not automatically imply OFF; staffing requirement remains explicit.
            </span>
          </div>
        ...
        <div className="product-setting-row">
          ...
          <div>
            <strong>Holiday fixture</strong>
            <span>Operational requirement to be configured per shift.</span>
          </div>
  ...
  if (section === "notifications")
    return (
      <Surface padding="lg">
        <SectionHeading eyebrow="Awareness preferences" title="Notification behavior" />
        <div className="product-setting-toggle">
          <div>
            <strong>Schedule changes</strong>
            <span>Published correction, swap result, replacement.</span>
          </div>
          <input aria-label="Schedule change notifications" defaultChecked type="checkbox" />
        </div>
        <div className="product-setting-toggle">
          <div>
            <strong>Payroll attention</strong>
            <span>Dirty state, finalize readiness, lock completion.</span>
          </div>
          <input aria-label="Payroll notifications" defaultChecked type="checkbox" />
        </div>
        <div className="product-setting-toggle">
          <div>
            <strong>Resolved operational warnings</strong>
            <span>Keep resolved items grouped instead of deleting them.</span>
          </div>
          <input aria-label="Resolved warning notifications" defaultChecked type="checkbox" />
        </div>
      </Surface>
    );
  return (
    <Surface padding="lg">
      <SectionHeading
        ...
        title={section === "payroll" ? "Payroll policy" : "General settings"}
      />
      <div className="product-form-grid">
        <label>
          Workspace name
          <input defaultValue="NOCScheduler" />
        </label>
        <label>
          Timezone
...
function SettingsSurface({ meta, pathname }: { meta: RouteMeta; pathname: string }) {
  ...
  return (
    <ProductPage meta={meta}>
      <section className="product-settings-layout">
...
function NotificationsSurface({ meta }: { meta: RouteMeta }) {
  return (
    <ProductPage
      actions={<HeaderAction href="/settings/notifications">Preferences</HeaderAction>}
      meta={meta}
    >
      <div className="product-toolbar">
        <div className="product-segmented">
      ...
      <Surface className="product-notification-list" padding="none">
        {notificationsFixture.map((notification) => (
          <Link
            className="product-notification-row"
            data-state={notification.state}
            href={notification.href}
            key={notification.id}
          >
            <span className="product-notification-icon">
              <Icon
                name={
                  notification.group === "Payroll"
                    ? "wallet"
                    : notification.group === "Schedule"
                      ? "calendar"
            ...
            <div>
              <div className="product-row-title">
                ...
                <Badge tone={toneForStatus(notification.state)}>{notification.state}</Badge>
              </div>
              <p>{notification.detail}</p>
              <span>
                {notification.group} · {notification.time}
              </span>
            </div>
            <Icon name="chevron-right" size={18} />
          </Link>
  ...
  return (
    <ProductPage meta={meta}>
      <Surface className="product-profile-hero" padding="lg">
        <Avatar initials={employee.initials} name={employee.name} />
        <div>
          <span className="product-eyebrow">Signed-in fixture</span>
          <h2>{employee.name}</h2>
          <p>raka.pratama@internal.example · NOC Member</p>
        </div>
        <Badge tone="success">Active</Badge>
      </Surface>
      <div className="product-summary-grid">
        <Surface padding="lg">
          ...
          <dl className="product-definition-grid">
            ...
            <div>
              <dt>Theme</dt>
              <dd>Stored browser preference</dd>
            </div>
          </dl>
        </Surface>
        <Surface padding="lg">
          <SectionHeading eyebrow="Security boundary" title="Authentication arrives in F05" />
          <p className="product-muted">
            This profile is a typed frontend fixture only. Session, password, revoke, and step-up
            controls remain intentionally deferred.
          </p>
        </Surface>
      </div>
    </ProductPage>
...
export function ProductSurface({ meta, pathname }: { meta: RouteMeta; pathname: string }) {
  ...
  if (pathname === "/schedule/manage" || pathname.startsWith("/schedule/manage/"))
    return <ManageScheduleSurface meta={meta} />;
  if (pathname === "/schedule/requests" || pathname.startsWith("/schedule/requests/"))
    return <RequestsSurface meta={meta} pathname={pathname} />;
  if (pathname === "/employees") return <EmployeeListSurface meta={meta} />;
  if (pathname.startsWith("/employees/"))
    return <EmployeeDetailSurface meta={meta} pathname={pathname} />;
  if (pathname === "/payroll" || pathname.startsWith("/payroll/"))
    return <PayrollSurface meta={meta} pathname={pathname} />;
  if (pathname === "/reports" || pathname.startsWith("/reports/"))
    return <ReportsSurface meta={meta} pathname={pathname} />;
  if (pathname === "/activity" || pathname.startsWith("/activity/"))
    return <ActivitySurface meta={meta} pathname={pathname} />;
  if (pathname === "/settings" || pathname.startsWith("/settings/"))
    return <SettingsSurface meta={meta} pathname={pathname} />;
  ...
  return (
    <ProductPage meta={meta}>
      <Surface padding="lg">
        <SectionHeading eyebrow="Fixture surface" title={meta.title} />
        <p className="product-muted">
          Canonical route is available and shares the F03 product-surface system.
        </p>
      </Surface>
    </ProductPage>
  );
